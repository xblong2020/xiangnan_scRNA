...
gam_predictions <- as.data.table(readRDS(file.path(source_paths$processed, "figure5b_gam_predictions.rds")))[scenario == "primary"]
precedence <- fread(file.path(source_paths$metadata, "figure5e_precedence_probabilities.tsv"))[is_primary == TRUE]
cells <- as.data.table(readRDS(file.path(figure5_paths(root, create = FALSE)$processed, "figure5_oriented_cell_scores.rds")))[method == "main/consensus pseudotime"]
...
summary_landmark <- function(axis_name, landmark_name) {
  ...
  c(median = stats::median(finite), lower = unname(stats::quantile(finite, 0.025, type = 8)),
    upper = unname(stats::quantile(finite, 0.975, type = 8)), n = length(values),
...
# Main-figure boundary rule: bootstrap t10 is evaluated first.  Corrected
# onset_time is retained in source data and sensitivity outputs, never used as
# the formal left edge.  If t10 is unavailable, only the first five-point run
# satisfying the directional derivative/CI/0.25-SD contract is accepted.
resolve_six_panel_start <- function(prediction, t10) {
  t10_stable <- is_stable_landmark(t10)
  if (t10_stable) {
    return(list(start = t10[["median"]], start_lower = t10[["lower"]], start_upper = t10[["upper"]],
                method = "bootstrap_t10", boundary_status = "resolved",
                directional_time = NA_real_))
  }
  deriv <- central_derivative(prediction$pseudotime, prediction$fit)
  deriv_lower <- central_derivative(prediction$pseudotime, prediction$lower)
  deriv_upper <- central_derivative(prediction$pseudotime, prediction$upper)
  directional <- find_directional_activation(
    prediction$pseudotime, prediction$fit, deriv, deriv_lower, deriv_upper,
    direction = 1, min_change_sd = 0.25, min_run = 5L, baseline_fraction = 0.10
  )
  if (isTRUE(directional$found) && is.finite(directional$time)) {
    half_width <- if (is.finite(t10[["lower"]]) && is.finite(t10[["upper"]]))
      max(0.05, (t10[["upper"]] - t10[["lower"]]) / 2) else 0.05
    return(list(start = directional$time,
                start_lower = max(0, directional$time - half_width),
                start_upper = min(1, directional$time + half_width),
                method = "directional_activation_fallback", boundary_status = "resolved",
                directional_time = directional$time))
  }
...
apply_edge_fade <- function(profile, observed_end = 1, edge_start = 0.82) {
  if (!nrow(profile)) return(profile)
  edge_fade_start <- max(min(profile$pseudotime, na.rm = TRUE), min(edge_start, observed_end - 0.02))
  edge_fade <- ifelse(profile$pseudotime <= edge_fade_start, 1,
                      pmax(0.12, 1 - 0.88 * (profile$pseudotime - edge_fade_start) /
                             max(observed_end - edge_fade_start, 1e-6)))
  profile[, fade := edge_fade]
  profile[, fade_start := edge_fade_start]
  profile[, fade_end := observed_end]
  profile[, programme_prominence := pmin(1, pmax(0, programme_prominence * edge_fade))]
  profile[, alpha := pmin(0.78, pmax(0.04, 0.04 + 0.68 * programme_prominence))]
  profile
}
...
bands <- rbindlist(lapply(names(figure5_axis_score_columns), function(axis_name) {
  ...
  choice <- resolve_six_panel_start(prediction, t10)
  stable_decline <- is_stable_landmark(decline)
  profile <- build_figure5h_activity_profile(
    pseudotime = prediction$pseudotime, fitted = prediction$fit,
    start = choice$start, t50 = t50[["median"]], maximum_slope = maximum_slope[["median"]],
    decline_onset = if (stable_decline) decline[["median"]] else NA_real_, direction = 1
  )
  if (!stable_decline) profile <- apply_edge_fade(profile, observed_end = max(prediction$pseudotime, na.rm = TRUE), edge_start = 0.82)
  profile[, `:=`(
    axis = axis_name,
    ...
    maximum_slope = maximum_slope[["median"]], maximum_slope_lower = maximum_slope[["lower"]],
    maximum_slope_upper = maximum_slope[["upper"]],
    ...
    right_edge_rule = if (stable_decline) "fade after stable decline_onset" else "visual edge fade; no programme end estimated"
  )]
...
band_summary[, unresolved_x := 0.14]

band_polygons <- rbindlist(lapply(split(bands, bands$axis), function(values) {
  if (nrow(values) < 2L) return(NULL)
  rbindlist(lapply(seq_len(nrow(values) - 1L), function(i) data.table(
    axis = values$axis[[i]], segment_group = paste(values$axis[[i]], i, sep = "::"),
...
p <- ggplot() +
  ...
  geom_polygon(data = band_polygons,
               aes(x = x, y = y, fill = axis, alpha = alpha, group = segment_group), colour = NA) +
  ...
  geom_point(data = band_summary[is.finite(boundary_start)],
             aes(x = boundary_start, y = y, colour = axis), size = 2.0, shape = 21, fill = "white", stroke = 0.55) +
  geom_segment(data = band_summary[is.finite(t50)],
               aes(x = t50_lower, xend = t50_upper, y = y - 0.05, yend = y - 0.05),
  ...
  geom_text(data = band_summary[boundary_status == "boundary unresolved"],
            aes(x = unresolved_x, y = y, label = "boundary unresolved"), hjust = 0, size = 2.0, colour = lancet_palette[8]) +
  annotate("text", x = 0.5, y = 0.42, label = caption_text, fontface = "italic", size = 2.10, colour = lancet_palette[9]) +
  annotate("text", x = 0.5, y = 0.16, label = "Overlapping activity is a relative prominence model; no discrete programme end is estimated.",
  ...
  labs(title = "F  Conservative overlapping regulatory-activity model",
       subtitle = "Open circle: bootstrap t10 start; circle: t50; triangle: maximum slope; light dashed outlines show uncertainty",
...
boundaries <- rbindlist(list(
  band_summary[, .(
    ...
    source = "1,000 dataset-stratified corrected bootstrap replicates"
  )],
  state_med[, .(
...
figure5_write_tsv(boundaries, file.path(paths$metadata, "figure5_six_panel_F_phase_boundaries.tsv"))
figure5_write_tsv(band_summary, file.path(paths$metadata, "figure5_six_panel_F_activity_band_summary.tsv"))
out_dir <- file.path(paths$figures, "panel_F_activity_model")
outputs <- export_figure5_plot(p, file.path(out_dir, "figure5_six_panel_F_activity_model"), 13.5, 4.8)
figure5_write_json(list(
  panel = "5F",
  title = "Conservative overlapping regulatory-activity model",
  formal_band_left_boundary = "bootstrap t10 when stable; otherwise first persistent directional activation satisfying derivative direction, derivative 95% CI excluding zero, >=0.25 SD change and >=5 grid points; otherwise boundary unresolved",
  uses_onset_time_for_formal_band = FALSE,
  corrected_onset_retained_in_source_data = TRUE,
  early_reverse_direction_not_activation = TRUE,
  right_edge = "visual fade at the observed right edge; a stable decline_onset may start the fade, but no programme end is estimated",
  no_discrete_stage_rectangles = TRUE,
  uncertainty = "light dashed outlines and intervals",
...
), file.path(paths$metadata, "figure5_six_panel_F_report.json"))
dir.create(file.path(paths$processed, "plot_objects"), recursive = TRUE, showWarnings = FALSE)
saveRDS(p, file.path(paths$processed, "plot_objects", "figure5_panel_F.rds"))
