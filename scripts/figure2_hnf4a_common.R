...
figure2_get_arg <- function(flag, default) {
...
figure2_diverging <- c(low = "#3B4CC0", mid = "#F7F7F7", high = "#B40426")
...
figure2_save <- function(plot, figure_dir, stem, width, height, tiff = FALSE) {
...
figure2_norm_path <- function(path) enc2utf8(gsub("\\\\", "/", normalizePath(path, mustWork = FALSE)))

figure2_write_json <- function(value, path) {
  dir.create(dirname(path), recursive = TRUE, showWarnings = FALSE)
  write_json(value, path, pretty = TRUE, auto_unbox = TRUE, na = "null")
}
